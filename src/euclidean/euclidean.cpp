 ...
 *
 * ENCODER:
 *      Short press to change between selecting a prameter and editing the
 * parameter. Long press to change global settings like output mode (trigger,
 * gate, flip) and internal clock tempo. Double Click to reset all patterns back
 * to the first step. Hold & rotate to change current output channel pattern.
 *
...
const unsigned char pencil_gfx[] PROGMEM = {
    0x00, 0x00, 0x00, 0xc0, 0x01, 0xa0, 0x02, 0x60, 0x04, 0x40, 0x08, 0x80,
    0x11, 0x00, 0x22, 0x00, 0x64, 0x00, 0x78, 0x00, 0x70, 0x00, 0x00, 0x00};
...
void loop() {
  ...
  if (ui_trigger_active &&
      millis() > last_clock_input + UI_TRIGGER_DURATION_MS) {
  ...
  for (int i = 0; i < OUTPUT_COUNT; i++) {
    if (patterns[i].NextStep() == 1) {
      if (state.output_mode == FLIP) {
        hw.outputs[i].On() ? hw.outputs[i].Low() : hw.outputs[i].High();
      } else {
        hw.outputs[i].High();
      }
    }
...
void HandleDoubleClick(EncoderButton &eb) { Reset(); }
...
void HandleLongPress(EncoderButton &eb) {
  ...
  selected_mode = (selected_mode == UIMODE_PAGE) ? UIMODE_SELECT : UIMODE_PAGE;
  update_display = true;
}
...
void UpdateParameter(Direction dir) {
  // Convert the configured encoder direction to a single integer equivalent
  // value.
  ...
  case UIMODE_SELECT:
    if (static_cast<Parameter>(selected_param) == 0 && val < 0) {
      ...
      selected_param =
          static_cast<Parameter>((selected_param + val) % PARAM_LAST);
    }
    break;
...
void ChangeSelectedOutput(Direction dir) {
  ...
  case DIRECTION_DECREMENT:
    if (state.selected_out > 0)
      --state.selected_out;
    update_display = true;
    break;
  case DIRECTION_INCREMENT:
    if (state.selected_out < OUTPUT_COUNT - 1)
      ++state.selected_out;
    update_display = true;
    break;
  }
...
void DisplayOutputModePage() {
  ...
  hw.display.drawLine(0, 10, 128, 10, 1);
  // Draw output modes
  (state.output_mode == TRIGGER) ? hw.display.fillRect(12, 20, 8, 8, 1)
                                 : hw.display.drawRect(12, 20, 8, 8, 1);

  (state.output_mode == GATE) ? hw.display.fillRect(12, 32, 8, 8, 1)
                              : hw.display.drawRect(12, 32, 8, 8, 1);

  (state.output_mode == FLIP) ? hw.display.fillRect(12, 44, 8, 8, 1)
...
void DisplayClockPage() {
  ...
  if (state.internal_clock) {
    (state.tempo > 99) ? hw.display.setCursor(34, 50)
